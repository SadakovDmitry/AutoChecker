...
## Новая гибридная версия

Добавлена экспериментальная гибридная версия авторазметки:

```text
стабильные подпричины -> full yes/no
рискованные подпричины -> safe mode: только уверенные "да", остальное review
```

Задача гибридного роутера - заранее определить, где полная авторазметка может дать много ошибок, и для таких подпричин автоматически включить более осторожный режим.

Результат текущего backtest:

| Режим | Покрытие | Precision | Ошибки | Автострок |
|---|---:|---:|---:|---:|
| Safe-only `target_precision=0.80` | 37.32% | 89.54% | 32 | 306 |
| Гибридный роутер | 48.41% | 90.18% | 39 | 397 |

То есть гибридная версия автоматически размечает больше чатов: `397` строк вместо `306`, покрытие растет с `37.32%` до `48.41%`, при этом precision остается около `90%`.
...
Текущий вывод: гибридный роутер выглядит перспективнее safe-only, если цель - увеличить долю авторазметки без сильного падения precision. Основные ошибки пока дают несколько ВЗР-тематик, поэтому следующий шаг - отдельно настроить risk-правила для проблемных причин.
